...
<div class="content">
    ...
    <h1>SEROLOGY</h1>
    <h3>Examination Desired: Dengue Duo</h3>

    <p><strong>Patient:</strong> {{$patient->fullname}}</p>

    {!! Form::open(['url'=>"/patients/$patient->id/dengue-duo", 'method'=>'post']) !!}
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; grid-gap:15px">
        ...
        <p>
            {{Form::label('ns1','NS1')}}
            {{Form::text('ns1',null, ['class'=>'w3-input'])}}
        </p>
        <p>
            {{Form::label('igm','IGM')}}
            {{Form::text('igm',null, ['class'=>'w3-input'])}}
        </p>
        <p>
            {{Form::label('igg','IGG')}}
            {{Form::text('igg',null, ['class'=>'w3-input'])}}
        </p>
    </div>
